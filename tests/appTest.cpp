...
protected:
    ...
    void SetUp() override {
        std::ofstream ofs("data/terminal_test.txt", std::ios::trunc);
        ofs.close();

        out = new TerminalOutput();
        repo = new MemoryUsers("data/terminal_test.txt");

        // 2. Fixed constructors to match your specific .h files
        // Dereferencing pointers (*repo, *out) because headers expect references (&)
...
TEST_F(TerminalSimulationTest, TieBreakerCheck) {
    runCommand("add 1 10");
    runCommand("add 2 10 500");
    runCommand("add 3 10 200");
...
TEST_F(TerminalSimulationTest, WeightsInfluence) {
    runCommand("add 1 10 20");
    runCommand("add 2 10 20 300"); // Shares two items (Weight 2)
    runCommand("add 3 10 400");    // Shares one item (Weight 1)
...
// Test: User is not in the system
TEST_F(TerminalSimulationTest, UserNotFound) {
    runCommand("add 1 10 20");
    out->clear();
    runCommand("recommend 999 10"); // User 999 doesn't exist
    EXPECT_EQ(out->capturedText, "\n");
}

// Test: Product is not in the system
TEST_F(TerminalSimulationTest, ProductNotFound) {
    runCommand("add 1 10 20");
    out->clear();
    runCommand("recommend 1 999"); // No one saw 999
    EXPECT_EQ(out->capturedText, "\n");
}

// Test: No shared products between users
TEST_F(TerminalSimulationTest, NoSimilarity) {
    runCommand("add 1 10");
    runCommand("add 2 50"); // Different history
    out->clear();
    runCommand("recommend 1 10");
    EXPECT_EQ(out->capturedText, "\n");
}

// Test: Don't recommend products the user already has
TEST_F(TerminalSimulationTest, FilterOwnedProducts) {
    runCommand("add 1 10 20");
    runCommand("add 2 10 20 30");
    out->clear();
    runCommand("recommend 1 10");
    EXPECT_EQ(out->capturedText, "30\n"); // Should not show 20
}

// Test: Check the 10-limit cut-off
TEST_F(TerminalSimulationTest, MaxTenLimit) {
    runCommand("add 1 100");
    for(int i = 2; i <= 15; ++i) {
        std::string cmd = "add " + std::to_string(i) + " 100 " + std::to_string(1000 + i);
        runCommand(cmd);
    }
    out->clear();
    runCommand("recommend 1 100");
    
    // Count spaces to check limit
    int count = 0;
...
TEST_F(TerminalSimulationTest, PersistenceCheck) {
    runCommand("add 88 10 20");
    
    // Simulate restart by making a new Repo
    MemoryUsers repo2("data/terminal_test.txt");
    Recommend recCmd("", out, &repo2);
    
    runCommand("add 99 10 30");
...
// test: data survives program restart
TEST_F(TerminalSimulationTest, DataPersistenceAfterRestart) {
    // 1. first session: add data and close
    runCommand("add 1 100 200");
    
    // 2. second session: new repo instance from same file
    MemoryUsers repo2("data/terminal_test.txt");
    Recommend recCmd("", out, &repo2);
    
    // check if user 1 still exists
    out->clear();
    runCommand("add 2 100 300"); // peer user
    recCmd.setInput(" 1 100");
    recCmd.execute();
    
    // if 300 is recommended, user 1 was loaded correctly
